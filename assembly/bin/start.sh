...
if [ -n "$BITS" ]; then
    ...
    JAVA_MEM_OPTS=" -server -Xmx1024m -Xms512m -Xmn256m -XX:PermSize=256m -Xss256k -XX:+DisableExplicitGC -XX:+UseConcMarkSweepGC -XX:+CMSParallelRemarkEnabled -XX:+UseCMSCompactAtFullCollection -XX:LargePageSizeInBytes=128m -XX:+UseFastAccessorMethods -XX:+UseCMSInitiatingOccupancyOnly -XX:CMSInitiatingOccupancyFraction=70 -Ddubbo.registry.file=$DEPLOY_DIR/cache/dubbo-registry-Order-0.cache"
else
...
#打印控制台日志
nohup java $JAVA_OPTS $JAVA_MEM_OPTS $JAVA_DEBUG_OPTS $JAVA_JMX_OPTS -classpath $CONF_DIR:$LIB_JARS com.xiaoma.gateway.Application > $STDOUT_FILE 2>&1 &
#不打印控制台日志
#nohup java $JAVA_OPTS $JAVA_MEM_OPTS $JAVA_DEBUG_OPTS $JAVA_JMX_OPTS -classpath $CONF_DIR:$LIB_JARS com.xiaoma.gateway.Application > /dev/null 2>&1 &
